...
- Use `$MLPL` or the adjacent release binary; never install over stable tools
  or modify `../sw-mlpl`.
- Cover applicable boundary cases and document representation invariants,
  dynamic size, logical/copy complexity, and explicit loops. Do not claim
  structural sharing while composite values copy.
- Algorithm curricula remain in `../demo-algorithms`; design-pattern
  extraction is a separate later refactoring.
