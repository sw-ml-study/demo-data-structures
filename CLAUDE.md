# CLAUDE.md — data-structures-extraction
...
- Prefer the thin `justfile` recipes. Do not introduce a Makefile unless a
  concrete external integration explicitly requires it.
- Use `$MLPL` or the documented adjacent release binary. Do not install or
  replace stable sw-MLPL binaries and do not modify `../sw-mlpl`.
- Lessons are deterministic and self-checking, cover applicable empty,
  singleton, boundary, duplicate, and invalid cases, and document invariants,
  dynamic size, logical/copy complexity, and explicit loops.
- Do not claim structural sharing while the runtime copies composite values.
  Use numeric IDs/handles for logical cycles and static strings for messages.
- Searching, sorting, graph, numeric, dynamic-programming, serialization, and
  matrix algorithms belong in `../demo-algorithms`. Design-pattern extraction
  is a separate later refactoring.
